...
export class ClaimService extends BaseApiService<Claim, number> {
  ...
  override getAll(includeAuth: boolean = true): Observable<Claim[]> {
    return super.getAll(includeAuth);
  }

  override getById(id: number, includeAuth: boolean = true): Observable<Claim> {
    return super.getById(id, includeAuth);
  }

  override create(claim: Omit<Claim, 'id'>, includeAuth: boolean = true): Observable<Claim> {
    return super.create(claim, includeAuth);
  }

  override update(claim: Claim, includeAuth: boolean = true): Observable<Claim> {
    return super.update(claim, includeAuth);
  }

  override delete(id: number, includeAuth: boolean = true): Observable<void> {
